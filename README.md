...
These are really all the options we need to care about; so all together my  command would be:<br />
...
Run this command with your desired configuration to prep the build system.
...
to install the compiled toolchain into `llvm-build`
...
In short, object files contain the machine code (bytecode), metadata, relocation instructions, and other program symbols generated by the compiler per **translation unit** (preprocessed source file).
...
But how do we get from point A to point B? In other words, after the compiler [front end](https://en.wikipedia.org/wiki/Compiler#Front_end) generates an internal map of the C code, known as an Abstract Syntax Tree,
...
"middle man" that novel languages can transcribe into to facilitate easy transformation operations (like optimizations, dead code elimination, etc.) and generation of object code,
effectively serving as the [middle and back end of the compiler](https://en.wikipedia.org/wiki/Compiler#Middle_end).
> This is exactly what the Clang compiler does! Assuming you built
...
> Then the **linker** patches these two obj files together to satisfy the external dependency of `fib` referenced in `main.cpp`
...
## Generated IR
This is without a doubt the most difficult and confusing part of this whole ordeal.
...
The generated output of this program is set to mimic `handwritten_fibonacci.ll`. I recommend splitscreening these two files to trace what the C++ code is generating.
...
The `Module` effectively represents a source file or **translation unit** in C/.ll code. Next we get a handle to a basic 32-bit signed integer type because it is the only type on which our function operates.

### Function declaration & definition

We declare the fib function in IR by decribing the function signature in `FunctionType` and creating an instance of the signature in a `Function`.
We store both pieces of data in a struct, `Function_Pair` because we will need to reference both when encoding the *function call* later.
...
> Hopefully you can start to see how arbitrary programs can be created outside the context of this predetirmined example leveraging the API and
> creating new `BasicBlock`s on the fly, or inserting instructions based on program operators, etc.

### Output to object file

After this step the program takes a little break to print the contents of our `Module` which includes our newly defined `fib` function in textual IR format.
...
Run the `llvm-config` tool from your terminal to verify it was installed correctly. This is your new best friend.
It will give your compiler context about where everything is installed on your system.
> **Note that the compiler this tool expects is the same compiler you used to build LLVM.**
...
`llvm-config --cxxflags --system-libs --libs all`<br/> Next we pass this output to our compiler **Remember: this part is specific to you!** For example:<br />
`cl -MD $llvm_info generate_fibonacci.cpp -Fegen_fib.exe`<br />
> For those who don't know, the -MD flag specifies that the C Runtime Library (CRT) should be linked statically, because this is what LLVM expects on Windows.

Assuming everything compiled without error, let's run `gen_fib.exe` to get our `fibonacci.obj` file.

> You should now see the IR dump of our `Module` in the terminal. Check back with `handwritten_fibonacci.ll`, is it identical? What is different? What is similar?
...
This sure has been a journey, hasn't it? Now you're ready to tame the beast on your own. Give yourself a pat on the back. This was definitely far from easy. Good luck soldier.